...
/// Well-known Newznab-compatible indexer presets.
///
/// Users still need to supply their own API key — only name and URL are
/// pre-filled.
const _indexerPresets = [
  ...
  (name: 'NZB.su', url: 'https://api.nzb.su'),
  (name: 'Tabula Rasa', url: 'https://www.tabula-rasa.pw'),
  (name: 'AnimeTosho', url: 'https://feed.animetosho.org'),
  (name: 'NZBnoob', url: 'https://api.nzbnoob.com'),
  (name: 'GingaDADDY', url: 'https://www.gingadaddy.com'),
];
...
class _IndexerFormDialogState extends ConsumerState<_IndexerFormDialog> {
                ...
                if (!_isEditing) ...[
                  SizedBox(
                    ...
                    child: OutlinedButton.icon(
                      ...
                      label: const Text('Choose from presets'),
  ...
  Future<void> _testConnection() async {
    ...
    if (url.isEmpty || (!_isEditing && apiKey.isEmpty)) {
      setState(() => _testResult = '✗ Please fill in URL and API key first');
      return;
    }
    ...
    try {
      ...
      if (_isEditing && apiKey.isEmpty) {
        result = await usenet.testIndexer(widget.existing!.uri);
      } else {
        result = await usenet.testIndexerDirect(
          baseUrl: url,
          apiKey: apiKey,
        );
      }
    ...
    if (!_tested) {
      final proceed = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          title: const Text('Connection not tested'),
          content: const Text(
            'The connection has not been tested yet. Save anyway?',
          ),
          actions: [
            TextButton(
              onPressed: () => Navigator.of(ctx).pop(false),
    ...
    showModalBottomSheet<void>(
      ...
      builder: (ctx) => ListView(
        ...
        children: [
          ...
          for (final preset in _indexerPresets)
            ListTile(
...
class _ProviderFormDialogState extends ConsumerState<_ProviderFormDialog> {
  ...
  Future<void> _testConnection() async {
    ...
    if (host.isEmpty || username.isEmpty) {
      setState(
        () => _testResult = '✗ Please fill in host and username first',
      );
      return;
    }
    if (!_isEditing && password.isEmpty) {
      setState(() => _testResult = '✗ Please fill in the password first');
      return;
    }
    ...
    try {
      ...
      final Result<bool> result;
      if (_isEditing && password.isEmpty) {
        result = await usenet.testProvider(widget.existing!.uri);
      } else {
        result = await usenet.testProviderDirect(
          host: host,
          port: int.tryParse(portText) ?? (_ssl ? 563 : 119),
          ssl: _ssl,
          username: username,
          password: password,
        );
